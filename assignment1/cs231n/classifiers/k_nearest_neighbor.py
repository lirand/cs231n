...
class KNearestNeighbor(object):
    ...
    def compute_distances_no_loops(self, X):
        ...
        dists = np.sqrt(te.reshape(-1,1) + mid + tr.reshape(1,-1))
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            closest_neigbours_idx = np.argsort(dists[i,:])[:k]
            
            for idx in closest_neigbours_idx:
            ...
            #closest_y_cnt = Counter(closest_y)
            #y_pred[i] = max(closest_y, key=closest_y_cnt.get)
