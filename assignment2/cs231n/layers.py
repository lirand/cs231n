...
def relu_forward(x):
    ...
    zeros_matrix = np.zeros((x.shape))
    out = np.maximum(zeros_matrix, x)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (x, out, gammax, xhat, istd, std, var, sq, xmu, mu, beta, gamma, eps, axis)
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    (x, out, gammax, xhat, istd, std, var, sq, xmu, mu, beta, gamma, eps, axis) = cache
    N, D = dout.shape
    #Step 9
    ...
    dgamma = np.sum(dgammax*xhat, axis=axis)
    dxhat = dgammax*gamma
    ...
    # print(f'dgammax.shape = {dgammax.shape}')
    # print(f'dgamma.shape = {dgamma.shape}')
    # print(f'dxhat.shape = {dxhat.shape}')
    # print(f'distd.shape = {distd.shape}')
    # print(f'dxmu.shape = {dxmu.shape}')
    # print(f'dstd.shape = {dstd.shape}')
    # print(f'dvar.shape = {dvar.shape}')
    # print(f'dsq.shape = {dsq.shape}')
    # print(f'dsq.shape = {dxmu2.shape}')
    # print(f'dsq.shape = {dmu.shape}')
    # print(f'dsq.shape = {dx1.shape}')
    # print(f'dsq.shape = {dx2.shape}')
    # print(f'dsq.shape = {dx.shape}')
...
def batchnorm_backward_alt(dout, cache):
    ...
    (x, _, _, xhat, _, _, var, _, _, mu, beta, gamma, eps, _) = cache
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    #print(x.T.shape, gamma.reshape(-1, 1).shape, beta.reshape(-1,1).shape)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    F_col = w.reshape(F, C*HH*WW)
    for sample in range(N):
      col = 0
      for i in range(0, h_pad-HH + 1, stride):
        for j in range(0, w_pad-WW +1, stride):          
          X_col[:,col] = x_pad[sample, :, i : i+HH, j : j+WW].reshape(C*HH*WW)
          col+=1
      
      out_temp = F_col.dot(X_col) + b.reshape((F,-1))
      out[sample] = out_temp.reshape(F, Hout, Wout)
...
def conv_backward_naive(dout, cache):
    ...
    for n in range(N):
      dx_pad = np.pad(dx[n,:,:,:], ((0,0), (pad,pad), (pad,pad)), 'constant' ) #(C, H, W)
      x_pad = np.pad(x[n,:,:,:], ((0,0), (pad,pad), (pad,pad)), 'constant' )   #(C, H, W)
      for f in range(F):
        for i in range(Hout):
          for j in range(Wout):
            h1 = i * stride
            ...
            w1 = j * stride
            w2 = j * stride + WW
            dx_pad[:, h1:h2, w1:w2] += w[f,:,:,:] * dout[n,f,i,j]
            dw[f,:,:,:] += x_pad[:, h1:h2, w1:w2] * dout[n,f,i,j]
            db[f] += dout[n,f,i,j]
      dx[n,:,:,:] = dx_pad[:,1:-1,1:-1]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, C, H, W = x.shape
    x2dim = x.transpose(0,2,3,1).reshape(H*W*N,C)
    out, cache = batchnorm_forward(x2dim, gamma, beta, bn_param)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dout2dim = dout.transpose(0,2,3,1).reshape(N*H*W, C)
    dx, dgamma, dbeta = batchnorm_backward(dout2dim, cache)
